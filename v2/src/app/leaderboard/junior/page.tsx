  ...
  useEffect(() => {
    const loadLeaderboardData = async () => {
      try {
        ...
        // Load junior players data
        const response = await fetch(`/junior-ratings.json?t=${Date.now()}`);
        if (!response.ok) {
          throw new Error('Failed to load junior players data');
        }
      ...
      <div className="font-sans min-h-screen bg-gradient-to-br from-blue-50 to-blue-200">
        ...
        <div className="bg-white min-h-screen flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-gray-600">Loading junior leaderboard...</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="bg-gradient-to-r from-green-400 to-green-600 text-white py-8">
          <div className="max-w-7xl mx-auto px-4">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-3xl font-bold">Junior Leaderboard</h1>
                <p className="text-green-100 mt-2">
                  {filteredPlayers.length} players • {sortedPlayers[0]?.tournamentCount || 0} tournaments max participation
                </p>
              </div>
              <Link href="/leaderboard" className="bg-white text-green-600 px-4 py-2 rounded-lg font-semibold hover:bg-green-50 transition-colors">
                ← Back
              </Link>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="mt-8 bg-green-50 rounded-lg p-6">
            ...
            <p className="text-green-800 mb-4">
              These young players have shown exceptional dedication by participating in multiple junior tournaments throughout the year, 
              demonstrating their growing skills and passion for chess.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-green-700">
              <div>
                ...
                <ul className="list-disc list-inside mt-1 ml-4">
                  ...
                  <li>Junior category tournaments</li>
